...
namespace TaskManager1._0
{
    ...
    {
        public frmLocalizarFuncionario()
        {
            InitializeComponent();
        }
        ...
        {
            tB_USUARIOBindingSource.Filter = "NOME_USUARIO like '" + txtNomeLocal.Text + "%'";
        }
        ...
        private void tB_USUARIODataGridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                VariaveisGlobais.CodigoTroca = int.Parse(tB_USUARIODataGridView.Rows[e.RowIndex].Cells[0].Value.ToString());
            }
            catch (Exception)
            {
